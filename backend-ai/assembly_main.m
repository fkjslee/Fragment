function [T_calc, confidence_calc]=assembly_main(seriesname,parts)
tic;
pics_dir='.'; % folder path that contains the fragments images
% seriesname='botanical23'; % name of the fragments series (<series_name>_p<frag_num><ver><ext>)
% seriesname='hovav_x';
% parts=[1, 8]; % the fragment numbers range for the assembly calculation (e.g. 1:18)
...
% parts=str2double(parts);hovav_x_p1_m
full_pics = arrayfun(@(x) {[seriesname, '/m', '/fragment_',num2str(x, '%04d'),'_m.png']},parts);
% full_pics = arrayfun(@(x) {[seriesname, '/m', '/hovav_x_p',num2str(x),'_m.png']},parts);
%-----------init rotations----------
n_rot = 80;
...
% TODO change this flag to false
load_options.is_use_rand_frag_rot = false; % rotate each fragment by a random angle
...
% T_tmp = [finalTr finalRot];
% disp(T_tmp);
% input was padded 10 around original image
% gt_img=d_Tforms2im(T_tmp, input);
% figure;imshow(gt_img);

%[finalsz, cn_offset] = s_find_transl_limits(input, T_tmp);
%T_tmp(:,1:2) = T_tmp(:,1:2) + cn_offset;
%fullmask = false(finalsz);
%finalsz = size(fullmask);
%sind2d_rc = round(bsxfun(@plus,mask_indices(input{2}.mask,finalsz),T_tmp(1:2)));
%disp(sind2d_rc);
...
% totalconf(1)
% [finalTr finalRot]
T_calc = [finalTr finalRot];
confidence_calc = totalconf(1);
